...
(function () {
  ...
  R.labels = {
    types: { glazing: 'Пластиковое остекление', aluminum: 'Алюминиевое остекление', finish: 'Отделка окна' },
...
(function () {
  ...
  const fresh = () => ({ version: 1, client: { name: '', phone: '+7', address: '' }, step: 'start', items: [], draft: null, nextNumber: 1, balcony: freshBalcony(), works: [], discount: 0, discountMode: 'none', targetPrice: '', targetSignature: null, targetStale: false, returnToEstimate: false });
  ...
  const select = (path, label, options, blank = false) => `<label class="remok-field">${label}<select data-path="${path}">${blank ? '<option value="">Выберите вариант</option>' : ''}${Object.entries(options).map(([k, v]) => `<option value="${k}" ${get(path) === k ? 'selected' : ''}>${esc(v)}</option>`).join('')}</select></label>`;
  ...
  function clientHTML() {
    return `<section class="remok-card"><h2>Объект замера</h2><p class="remok-help">Данные клиента необязательны. Можно сразу перейти к расчёту.</p><div class="remok-grid">${input('client.name', 'Клиент', 'text', 'Имя клиента')}${input('client.phone', 'Телефон', 'tel', '', 'autocomplete="tel" inputmode="tel" aria-describedby="remok-phone-help"')}<p id="remok-phone-help" class="remok-help remok-full">Формат: +7 XXX XXX-XX-XX. Введите 10 цифр после +7.</p><div class="remok-full">${input('client.address', 'Адрес объекта', 'text', 'Улица, дом, квартира')}</div></div></section>`;
  }
  function startHTML() {
    const adding = state.chooseProduct || state.items.length || state.balcony.saved;
    const choices = [['glazing', 'Пластиковое остекление', 'Окно ПВХ с возможной отделкой'], ['aluminum', 'Алюминиевое остекление', 'Холодное алюминиевое остекление'], ['finish', 'Отделка окна', 'Внутренняя и/или наружная отделка существующего окна']];
    if (!adding) choices.push(['balcony', 'Отделка балкона', 'Внутренняя отделка балкона']);
  ...
  function editorHTML() {
    ...
    if (dimensionsReady) {
      if (d.mode === 'glazing' || d.mode === 'aluminum') {
        ...
        if (price !== null) html += `<div class="remok-result"><span class="remok-success">✓ Остекление</span>${linesHTML([{ title: C.glazingDetail(d), detail: `${d.width} × ${d.height} мм · Площадь: ${decimal(C.area(d.width, d.height))} м²`, price }])}</div><div class="remok-section">${yesno('draft.exterior.enabled', 'Нужна наружная отделка?')}</div>`;
  ...
  window.addEventListener('pageshow', () => { pricing = R.storage.pricing(); render(true); });
  window.addEventListener('storage', e => { if (e.key === 'remok.estimator.pricing.v1') { pricing = R.storage.pricing(); render(true); notify('Цены обновлены'); } });
  render(); persist();
})();
  }
})();
