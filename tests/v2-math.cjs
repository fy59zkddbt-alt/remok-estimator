...
let r=C.product(item(),p);near(r.total,63000);near(r.lines[0].price,54000);near(r.lines[1].price,9000);
for(const [lam,total] of [['none',54000],['one',70200],['two',81000]]){r=C.product({...item(),lamination:lam},p);near(r.total,total+9000);near(r.lines[0].price,total);near(r.installationDisplayPrice,9000);}
...
r=C.product(block,p);near(r.area,3.64);near(r.total,3.64*(18000+3000));near(r.installationDisplayPrice,10920);assert.deepEqual(JSON.parse(JSON.stringify(V.finishDimensions(block))),{width:2100,height:2200});
const partial=C.product({...block,windowHeightMm:''},p);assert(partial.errors.length);near(partial.total,0);
block.exterior.enabled=true;near(C.product(block,p).total,3.64*(18000+3000)+C.calculateExterior(2100,2200,180,'aquilon',p));
const legacy={...item(),productType:'balcony_small',width:2100,height:2200};near(C.product(legacy,p).total,4.62*(18000+3000));assert(!V.modernBlock(legacy));
