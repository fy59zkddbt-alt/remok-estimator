...
for(const [w,h] of [[780,2050],[1600,1400],[2400,1200]]){
 ...
 near(C.product(a,p).total,w*h/1e6*18000);
}
...
for(const widths of [[1400],[700,700],[500,1000]])for(const position of widths.length===1?['left','right']:['left','middle','right']){
 ...
 const expected=(widths.reduce((s,w)=>s+w,0)*1500+700*2200)/1e6;near(C.product(a,p).area,expected);near(C.product(a,p).total,expected*18000);near(V.finishDimensions(a).width,widths.reduce((s,w)=>s+w,0)+700);near(V.finishDimensions(a).height,2200);
 ...
 a.exterior={enabled:true,type:'aquilon',depth:180,depthChecked:true};near(C.product(a,p).total,expected*18000+C.calculateExterior(V.finishDimensions(a).width,2200,180,'aquilon',p));
}
const legacy={...item(2100,2200),productType:'balcony_small',sections:[{widthMm:1400,openingType:'fixed'},{widthMm:700,openingType:'turn'}]},snapshot=JSON.stringify(legacy);near(C.product(legacy,p).total,4.62*18000);S.geometry(legacy);assert.equal(JSON.stringify(legacy),snapshot);assert(!V.modernBlock(legacy));
