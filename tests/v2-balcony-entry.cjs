...
(async()=>{
 ...
 await f('balcony.floor.enabled').check();await f('balcony.floor.length').fill('2000');await f('balcony.floor.width').fill('1500');
 const baseTotal=await page.evaluate(()=>Remok.calc.balcony(Remok.storage.read('estimate').balcony,Remok.storage.pricing()).total);assert(baseTotal>0);
 await page.locator('[data-action="add-work"][data-scope="balcony.additionalWorks"]').click();await f('balcony.additionalWorks.0.name').fill('Герметизация балкона');await f('balcony.additionalWorks.0.amount').fill('1500');await click('save-balcony');
 assert.equal(await page.locator('h2').filter({hasText:/^Отделка балкона$/}).count(),1);assert.equal(await page.locator('[data-action="add-product"]').count(),1);assert.equal(await page.locator('[data-action="continue"]').count(),1);
 ...
 const data=await page.evaluate(()=>testPdfData);assert.equal(data.balcony.total,baseTotal+1500);assert.equal(data.totals.sub,baseTotal+1500);assert(data.balcony.lines.some(l=>l.label.includes('Герметизация балкона')&&l.price===1500));
 ...
 for(const mode of ['glazing','balcony-glazing']){
  ...
  await f('draft.mode').selectOption('balcony');assert.equal(await page.locator('h1').innerText(),'Отделка балкона');assert.equal(await f('balcony.floor.length').inputValue(),'2000');assert.equal(await f('balcony.additionalWorks.0.amount').inputValue(),'1500');
  const state=await page.evaluate(()=>Remok.storage.read('estimate'));assert.equal(state.step,'balcony');assert.equal(state.draft.mode,mode);
  await click('save-balcony');await click('add-product');await five();
 }
